    ...
    if (tv == users)
    {
        ...
        if (row < [acceptedUsers count])
        {
            if ([[column identifier] isEqualToString:@"username"])
                ...
                return [NSNumber numberWithInt:[backendMenu indexOfItemWithTitle:[[acceptedUsers objectAtIndex:row] objectForKey:@"backend"]]];
        }
        else
            if (![[column identifier] isEqualToString:@"username"])
...
- (void) savePreferences
{
    if (savedllconf)
    {
        char tmpfilec[17]; strcpy (tmpfilec, "/tmp/sane.XXXXXX"); mktemp (tmpfilec);

        FILE * f = fopen (tmpfilec, "w");
...
- (IBAction) buttonPressed:(id) sender
{
    ...
    else if (sender == sanedHelp)
    {
        ...
        char tmpfilec[22]; strcpy (tmpfilec, "/tmp/sane.XXXXXX.html"); close (mkstemps (tmpfilec, 5));
        ...
        [task setStandardOutput:[NSFileHandle fileHandleForWritingAtPath:tmpfile]];
        [task launch];
        [task waitUntilExit];
...
- (IBAction) buttonCellPressed:(id) sender
{
    if (sender == backends)
    {
        ...
        if ([sender clickedColumn] == [sender columnWithIdentifier:@"configure"])
        {
            ...
            int result = [NSApp runModalForWindow:configSheet];
